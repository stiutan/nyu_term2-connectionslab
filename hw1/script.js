...
const beatImages = new Map(
  beatImageSources.map((src) => {
...
const state = {
  timeSig: 4,
  beats: ["empty", "empty", "empty", "empty", "empty", "empty"],
  bpm: 100,
  isPlaying: false,
};
...
document
  .querySelectorAll(".topbar-maincontrols-ingredientsdropdown")
  .forEach((button) => {
    button.addEventListener("click", () => {
      const isPressed = button.getAttribute("aria-pressed") === "true";
      button.setAttribute("aria-pressed", String(!isPressed));
      document
        .querySelector(".ingredient-list-expanded")
        .setAttribute("aria-hidden", String(isPressed));
    });
  });

const servingSizeOption = document.querySelectorAll(
  ".topbar-servingsize-option",
);
...
document.querySelectorAll(".beat-options button").forEach((button) => {
  button.addEventListener("click", async () => {
    ...
    document
      .querySelectorAll(`.content-beat[data-beat="${beatNum}"]`)
      .forEach((beat) => {
        ...
        beat.querySelectorAll(".beat-options button").forEach((choice) => {
          choice.setAttribute(
            "aria-pressed",
            String(choice.dataset.ingredient === ingredient),
          );
        });
      });
